...
from lava.lib.optimization.solvers.lca.process import LCA1Layer

from lava.lib.optimization.solvers.lca.util import apply_activation

class TestLCA1LayerLoihi2(unittest.TestCase):
    @unittest.skip("Zero weights aren't supported yet.")
    def test_identity_matrix(self):
        weights = np.zeros((5, 5), dtype=np.int8)
        weights_exp = -8
        input_val = np.array([6502, 29847, 14746, 8168, 12989])
        threshold = 1
        lca = LCA1Layer(weights=weights, weights_exp=weights_exp,
                        bias=input_val, threshold=threshold)
    ...
    def test_competition(self):
        ...
        self.assertTrue(np.allclose(expected, actual, atol=20_000, rtol=1e-3),
